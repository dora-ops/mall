  ...
  export default {
    ...
    mounted () {
      productHome().then(res => {
        ...
        let homfloors=home_floors.filter(item=>{
          return item.title!='净化器及配件'&&item.title!='品牌精选'&&item.title!='官方精选'
        })
         let item=this.cloneObj(homfloors[0]);
        item.title='品牌精选'
       
        homfloors.push(item)
      
         let item1=this.cloneObj(homfloors[0])
         item1.title='官方精选'
         
        homfloors.push(item1)
        this.floors = homfloors
        this.hot = home_hot
      })
    },
    components: {
